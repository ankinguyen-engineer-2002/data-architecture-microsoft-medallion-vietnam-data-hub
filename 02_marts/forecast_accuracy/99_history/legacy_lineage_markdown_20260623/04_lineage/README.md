...
> **Source correction (2026-06-23):** live SQL/registry check found no active Forecast Accuracy runtime dependency on `SupplyChain_Lakehouse`. Any older EDW/Dataflow Gen2 edge in pre-2026-06-23 exports should be treated as stale history.
...
Enterprise_Lakehouse.SalesHistory_AFI.{InvoiceDetail, InvoiceHeader}
Enterprise_Lakehouse.SupplyChain_Enh.DemandForecastSnapshotDaily
ProcessingSeed.{ForecastCycle, ForecastHorizon}
```

### Layer 1: Staging / Wrk ← External Bronze

| Target | ← Source(s) |
|--------|-------------|
| `SalesHistory_Enh.InvoiceDetailLineLevel` | `Enterprise_Lakehouse.SalesHistory_AFI.InvoiceDetail`, `Enterprise_Lakehouse.SalesHistory_AFI.InvoiceHeader` |
...
| Target | ← Source(s) |
|--------|-------------|
| `SalesHistory_Enh.InvoiceDetailLineLevel` | `Enterprise_Lakehouse.SalesHistory_AFI.InvoiceDetail`, `Enterprise_Lakehouse.SalesHistory_AFI.InvoiceHeader`, `ReferenceMaster_Enh.CustomerAccountGroup` |
...
Reads CSVs from `03_operations/apps/lineage_explorer/data/` (refreshed every 10 min by GitHub Action `.github/workflows/refresh_lineage_data.yml`).
