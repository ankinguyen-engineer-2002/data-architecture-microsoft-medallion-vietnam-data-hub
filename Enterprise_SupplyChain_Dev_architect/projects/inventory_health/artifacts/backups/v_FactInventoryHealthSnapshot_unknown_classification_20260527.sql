...
CREATE OR ALTER VIEW InventoryHealth_DW.v_FactInventoryHealthSnapshot AS
...
current_supply_keys AS (
    -- DA/BRD current PO/MO supply can exist without an on-hand inventory row.
    -- Keep inventory base narrow, but add nonzero PO/MO item-location keys so Gold measures do not drop them.
...
base AS (
    ...
    -- Current supply-only rows: PO/MO exists but no current inventory base row.
    SELECT
        CAST('Current'                          AS VARCHAR(10))  AS SnapshotType,
        csk.SnapshotDate                                         AS SnapshotDate,
        csk.ItemSku,
        csk.WarehouseCode,
        CAST(0 AS DECIMAL(18,4))                                  AS OnHandQty,
        CAST('InventoryHistory_Enh' AS VARCHAR(64))                AS SourceSystem,
        CAST('PO/MO supply-only key' AS VARCHAR(128))              AS SourceTable
    FROM current_supply_keys csk
    WHERE NOT EXISTS (
        SELECT 1
        FROM _InventoryCurrent ic
        WHERE ic.SnapshotDate = csk.SnapshotDate
          AND ic.ItemSku = csk.ItemSku
          AND ic.WarehouseCode = csk.WarehouseCode
    )
...
SELECT
    ...
    CAST(CASE
        WHEN dim.ItemSku IS NULL THEN 'Unknown'
    ...
    CAST(COALESCE(dim.LifecycleStatus, 'Unknown') AS VARCHAR(20)) AS LifecycleStatus,
    ...
    CAST(ISNULL(dim.UnavailableFlag, 0) AS BIT)           AS UnavailableFlag,
