...
> Mục tiêu: thay đổi framework vận hành từ VN Control Plane hiện tại sang mô hình nền tảng do BOB ETL Framework điều khiển, nhưng **không thay đổi business logic ETL**, **không redesign Medallion**, và **không phá 04_semantic/report contract**.
>
> Ngày đánh giá: 2026-06-21 ICT  
> Cập nhật định hướng: 2026-06-22 ICT — Phase 1 dùng local `Enterprise SupplyChain-Dev.ETL_Framework` làm nền, nâng lên full BOB ETL Framework, rồi apply 100% cho Bronze/Silver/Gold; Phase 2 mới port các năng lực VN Control Plane cần giữ.
...
> Cập nhật closeout: 2026-06-24 ICT — full 4-wrapper BOB runtime rerun đã pass, final audit pass, `TableDictionary`/`AuditLog` sạch cho active runtime, live/local SQL contract khớp `49/49`.
...
- [Verified] `EnterpriseData-Dev` là enterprise hub có BOB ETL Framework: `ETL_Framework.DW_Developer.TableDictionary`, `AuditLog`, `TableDictionary_UpdateLog`, nhiều loader/refresh procs, và pattern domain warehouse `_Wrk -> _LOAD -> final table`.
...
- [Likely] Kiến trúc mục tiêu hợp lý nhất sau cập nhật 2026-06-22 là **local `Enterprise SupplyChain-Dev.ETL_Framework` được nâng lên full BOB và làm primary runtime/load framework cho cả Bronze/Silver/Gold**; các năng lực VN cần giữ chỉ được port sau đó như BOB-compatible extensions: **DQ, Data Lineage, Wave Planner**.
- [Verified] `Meta.usp_GenericLoad` là capability hiện hữu và vẫn hữu ích cho rollback/fallback, nhưng không còn là target runtime nếu mục tiêu là “chuẩn y chang BOB”.
...
Chỉ thay:
  Runtime/load framework: local ETL_Framework được nâng lên full BOB và owns Bronze/Silver/Gold
  Hợp đồng metadata: local BOB TableDictionary là registry chính trong Enterprise SupplyChain-Dev
  Điểm vào orchestration: BOB wrapper/loader pattern là đường chạy chính
...
- [Verified] `00_CONTEXT/current.md`: active context log sau repo restructure; bản full trước khi split được giữ ở `00_CONTEXT/_source/CONTEXT_full_before_split_20260623.md`.
- [Verified] `01_docs/bob-framework/source/ETL_FRAMEWORK_GUIDE.md`, `01_docs/bob-framework/source/FABRIC_ARCHITECTURE_AND_STANDARDS.md`, `01_docs/bob-framework/source/SQLPROJ_BEST_PRACTICES.md`: tài liệu Bob mới gửi mô tả intended architecture, `.sqlproj` conventions, và canonical framework concepts như `AuditLog`, `TableDictionary`, `Performance_Logs`, SLA/email alerting.
- [Verified] `01_docs/bob-framework/source/EnterpriseData - ETL_Framework DataWarehouse Data Feed Alert_ 0.2857% behind (1 of 350).msg`: sample email thật từ `Datawarehouse Alerts`, ngày 2026-06-22, chứng minh BOB có data-feed/SLA alert output dựa trên TableDictionary freshness với fields `Hours Late`, `Last Updated (UTC)`, `Refresh Rate`, `Schema Name`, `Table Name`, `Job Server`, `Job Name`.
- [Verified] Đối chiếu repo ngày 2026-06-23 cho thấy `01_docs/bob-framework/source` hữu ích để hiểu intent/canonical style, nhưng **không phải** object-inventory source-of-truth 1:1 cho Phase 1; nó khác live reverse-engineered hub ở các điểm như 4-col `AuditLog`, `FabricMapping`, proc-family thực tế (`usp_RefreshCuratedTableFromView`, `usp_IncrementalTableLoad`, parquet variants) và absence của một số guide-only names trong evidence pack.
...
- [Verified] `01_docs/decisions/ADR-008-bob-alignment-naming-and-integration.md`: Bob alignment đã implemented; `Meta.TableDictionary`, `AuditLog`, `TableDictionary_UpdateLog` được clone/port local.
- [Verified] `99_archive/reverse-engineering/enterprise_data_architect/10_evidence/02_etl_framework_summary.md`: pattern BOB `ETL_Framework`, `TableDictionary` 65 cột, `AuditLog`, các nhóm loader, pattern refresh `_Wrk`.
...
- [Verified] `01_docs/runbook/artifacts/20260616_compare_ws/20260616_ws_compare_analysis.md`: so sánh trực tiếp VN control plane với BOB ETL Framework.
...
- [Verified] `EnterpriseData-Dev.ETL_Framework`: `TableDictionary=812`, `AuditLog=58261`, `TableDictionary_UpdateLog=23064`; đây là reference chuẩn BOB để diff/sync local framework.
...
Meta.TableDictionary + AuditLog + UpdateLog
  -> governance mirror tương thích Bob
```
...
- [Verified] Enterprise-facing presentation vẫn khó đọc hơn BOB `TableDictionary`.
...
- [Verified] `TableDictionary`: Bob-compatible governance mirror.
...
- [Verified] Hub-side BOB evidence có `Performance_Logs`, SLA alert/email queue families, và guide/email của Bob cũng nhắc data-quality alerting qua email; tuy nhiên những năng lực này hiện **chưa là prerequisite** để chứng minh current SupplyChain runtime/load path chạy được qua local BOB Phase 1.
...
[Need-verify] Production promotion path cần Bob/Rakesh xác nhận vì hiện `SupplyChain_Warehouse` đã tồn tại nhưng vẫn rỗng.
...
- SupplyChain-specific Gold và 04_semantic/report serving tiếp tục nằm trong value-stream workspace, trừ khi Bob/Rakesh quyết định khác.
...
```text
Local `Enterprise SupplyChain-Dev.ETL_Framework` được nâng lên full BOB ETL Framework và trở thành primary runtime/load framework cho Bronze/Silver/Gold.
VN business SQL giữ nguyên về mặt chức năng.
VN runtime intelligence không còn là runner chính; chỉ DQ, Data Lineage, Wave được port như BOB-compatible extensions.
...
Bronze access layer
  ...
  - local BOB TableDictionary + BOB loader/wrapper pattern là đường chạy chính
        |
        v
Silver layer
  - existing SQL views/business transformations được bảo toàn
  - `_Wrk` views và target tables chạy theo BOB ETL Framework
...
Gold layer
  ...
  - BOB loader/wrapper publish Gold theo cùng business SQL hiện có
        |
        v
Semantic layer
...
Enterprise SupplyChain-Dev
  ETL_Framework
    ...
    - bổ sung/sync missing BOB tables/views/procs để đạt full BOB contract
    ...
    - BOB wrapper/loader procs chạy Bronze/Silver/Gold
  ...
  SupplyChain_Processing_Warehouse
    ...
    - `_Wrk` views tương thích BOB khi cần
...
EnterpriseData-Dev
  ETL_Framework
    - source-of-truth reference cho full BOB contract
  ...
  SupplyChain_Warehouse
    ...
    - future placement nếu Bob/Rakesh quyết định move shared SupplyChain objects lên hub
    - không phải prerequisite của Phase 1 local BOB adoption
```
...
```text
BOB metadata registration
  -> BOB audit/update logs
  -> BOB loader/wrapper execution for Bronze/Silver/Gold
  -> Phase 2 BOB extensions for DQ / Data Lineage / Wave
```
...
|---|---|---|
| Enterprise catalog | BOB `TableDictionary` | object nào tồn tại, owner, source, refresh method, Modified/RowCount |
| Runtime contract | BOB `TableDictionary` + BOB load wrapper | load method, source view, target table, Modified/RowCount |
| Audit trail | BOB `AuditLog` + VN `RunLog` | enterprise message log + asset-level truth |
| Wave extension | BOB execution-wave table seeded from VN dependency logic | dependency-safe execution |
| Lineage extension | `LineageEdge` ported to BOB extension + Fabric/Purview lineage | impact analysis |
| Quality extension | `DQRule` / `DQGateRun` ported to BOB DQ extension | validation evidence |
...
- [Need-verify] Bob/Rakesh xác nhận owner của `EnterpriseData-Dev.SupplyChain_Warehouse`.
...
[Verified] BOB framework là governance surface được enterprise nhận diện trong `EnterpriseData-Dev`. Nếu SupplyChain muốn được promotion ở cấp enterprise, nó phải nhìn được qua metadata/audit paths tương thích BOB.
...
[Verified] VN control plane có các capabilities chưa thấy rõ trong BOB ETL Framework snapshots:
...
|---|---|---|---|---|
| Framework ownership | VN `Meta` là runtime brain | BOB framework là primary runtime/load framework | Cần chuyển runtime sang BOB, không chỉ làm facade | Cao |
| Metadata | VN `AssetRegistry` giàu hơn; BOB `TableDictionary` là enterprise registry | BOB `TableDictionary` là registry chính; VN metadata chỉ seed/fallback/extension | Cần mapping và cutover registry | Cao |
| Orchestration | VN có waves/due-gate; BOB có wrappers/FabricMapping | Phase 1 BOB wrapper/loader; Phase 2 Wave extension | Cần tách baseline BOB khỏi nâng cấp wave | Cao |
...
| Hub SupplyChain WH | đã tồn tại nhưng rỗng | Bronze/Silver/Gold objects được populate/approve theo BOB target scope | Cần object migration plan | Cao |
...
2. **Implement BOB 100% runtime/load pattern** cho Bronze/Silver/Gold trước.
3. **Port selectively** DQ, Data Lineage, Wave từ VN Control Plane sang BOB-compatible extensions.
4. **Retire/demote** các phần VN control dư thừa chỉ sau parity và rollback window.

### 6.2 Mapping Control Plane mục tiêu

| VN Control Plane | Mục tiêu dưới BOB migration | Quy tắc |
|---|---|---|
| `Meta.AssetRegistry` | transition inventory + rollback source | không còn primary runtime registry sau Phase 1 |
| `Meta.TableDictionary` | one-time/scheduled sync vào `ETL_Framework.DW_Developer.TableDictionary` | BOB `TableDictionary` là registry chính |
| `Meta.RunLog` | optional detail log / fallback evidence | không thay thế BOB `AuditLog`, chỉ bổ sung nếu cần |
| `Meta.PipelineRunLog` | reconciliation seed | sửa stuck-running rồi map vào BOB ops view nếu cần |
| `Meta.SilverDagWaveRuntime` | BOB Wave extension | port sau Phase 1 để giữ dependency-safe execution |
| `Meta.DQRule/DQGateRun` | BOB DQ extension | port sau Phase 1; không chặn BOB baseline cutover |
| `Meta.LineageEdge` | BOB Lineage extension + Fabric/Purview lineage | port sau Phase 1; giữ source-to-target graph |
...
- [Verified] BOB `TableDictionary` + wrapper/loader path chạy được Silver/Gold end-to-end qua 4 wrappers ngày 2026-06-24.
- [Verified] BOB runtime trả lời được: target table, source `_Wrk` view, last modified, status, duration/error qua `TableDictionary`, `AuditLog`, và `queryinsights`.
...
## 7. Kế hoạch áp dụng BOB ETL Framework

### 7.1 Mô hình áp dụng

Khuyến nghị cập nhật: **dùng local `Enterprise SupplyChain-Dev.ETL_Framework` làm nền, nâng nó lên full BOB contract, rồi dùng nó làm primary runtime/load framework 100% cho Bronze/Silver/Gold; VN chỉ đóng vai trò fallback/extension capability sau Phase 1**.

Clarification sau khi đọc `01_docs/bob-framework/source` ngày 2026-06-23:

- [Verified] `01_docs/bob-framework/source` nên được dùng như **canonical design guide / naming / deployment / intent reference**.
- [Verified] Phase 1 implementation target vẫn phải bám **live approved BOB runtime/load contract** đã reverse-engineer và verify trong repo/hub snapshots: `TableDictionary`, `AuditLog`, `TableDictionary_UpdateLog`, `FabricMapping`, curated refresh wrappers, incremental loaders, `Performance_Logs` surface liên quan.
...
- [Likely] Các phần Bob nêu thêm qua email như email-based DQ alerts, restart-from-failure semantics, lineage/catalog augmentation là capability thật của Bob ecosystem, nhưng với current SupplyChain scope chúng nên được classify rõ thành `Phase 1 observability follow-up` hoặc `Phase 2+ extension` nếu chưa cần để hoàn thành runtime/load cutover.

```text
Phase 1
  Reuse local ETL_Framework hiện có
  Diff/sync missing BOB framework objects từ EnterpriseData-Dev
  Local BOB TableDictionary / AuditLog / UpdateLog
  Execution manifest seeded from Meta.AssetRegistry + Meta.SilverDagWaveRuntime
  Local BOB _Wrk views / current business SQL views
  Local BOB loader/wrapper procs
...
Phase 2
  BOB DQ extension
  BOB Lineage extension
  BOB Wave Planner extension
```

### 7.2 Đường triển khai ưu tiên qua local `ETL_Framework`

[Verified] `Enterprise SupplyChain-Dev.ETL_Framework` đã tồn tại và có sẵn một phần BOB-style framework, nhưng hiện chưa đủ full BOB và đang trỏ legacy `SupplyChain_Warehouse.SCP_Core.*` / `Temp_SCPWarehouse.SCP_Core.*`.
...
2. Diff local `ETL_Framework` với `EnterpriseData-Dev.ETL_Framework` để xác định phần thiếu so với full BOB contract.
3. Sync/backfill missing BOB framework objects vào local `ETL_Framework` sau approval; không drop/recreate object cũ nếu chưa có explicit approval.
...
8. Bọc existing SQL business logic bằng BOB `_Wrk` view/wrapper pattern khi cần, nhưng không rewrite transformation logic.
9. Dùng local BOB loader/wrapper procs hoặc generated SQL run script để publish/refresh target tables.
...
11. So sánh output hiện tại với output qua BOB path.
...
Trong kế hoạch cập nhật, hub warehouse này **không phải prerequisite để apply Phase 1**. Nó là future placement/landing zone nếu Bob/Rakesh muốn promote một phần SupplyChain objects lên hub sau khi local BOB path đã chạy ổn.

Quy tắc:

- Phase 1 không phụ thuộc việc populate hub `SupplyChain_Warehouse`.
- Phase 1 lấy `EnterpriseData-Dev.ETL_Framework` làm reference chuẩn BOB để diff/sync framework contract.
...
- [Verified] Sequential BOB wrapper có thể dùng để establish Phase 1 baseline, nhưng không được xem là final replacement cho Wave Planner.
...
- [Verified] Không đưa `Meta.usp_GenericLoad` vào target path nếu mục tiêu là BOB 100%; nó chỉ là fallback/rollback.
...
- [Verified] Không tạo Fabric pipeline mới nếu Bob không approve. Phase 1 dùng SQL-only wrappers/manual scripts; nếu sau này Bob cho phép thì existing approved pipeline chỉ gọi wrapper đã có.
...
SQL view/proc VN hiện tại
  -> copy nguyên văn vào _Wrk view tương thích BOB hoặc target đã approve
  -> bọc bằng metadata/load mechanism của BOB
  -> so sánh outputs
```
...
### Giai đoạn 1 — Apply BOB ETL Framework 100% cho Bronze/Silver/Gold

Mục tiêu: dùng local `Enterprise SupplyChain-Dev.ETL_Framework` hiện có làm nền, nâng nó lên full BOB framework contract, rồi register/repoint current v10 Bronze/Silver/Gold vào local BOB path. Phase này là **một workstream duy nhất**: `reuse local ETL_Framework` + `complete full BOB framework` + `apply 100% BOB cho 3 Medallion layers`.
...
Input giữ nguyên
  ...
  -> BOB framework path thay thế VN runtime path
...
- Không port DQ/Data Lineage/Wave thành BOB extensions; để Phase 2.
- Không build dynamic BOB Wave engine; chỉ preserve current wave/order như static execution contract.
...
- Không tạo Fabric pipeline mới nếu Bob không approve.
- Không rebind report/semantic nếu chưa có PBIR/TMDL validation gate riêng.

Ước tính tổng thời gian Phase 1: **5-6.5 ngày làm việc** cho DEV baseline + side-by-side BOB path + manifest-based manual execution, tùy quyền deploy, số object cần register, và độ lệch giữa local `ETL_Framework` với full BOB. Các estimate dưới đây là [Need-verify] và phải được update sau read-only baseline pack.

**Progress snapshot — cập nhật 2026-06-23 13:25 ICT**

- [x] `Phase 1A` — Read-only baseline, rollback pack, artifact freeze đã hoàn tất.
- [x] `Phase 1B` — Diff local `ETL_Framework` với full BOB reference đã hoàn tất.
- [x] `Phase 1C` — `Option A` additive uplift cho local framework đã apply xong cho backlog cần trước mắt của Phase 1.
- [x] `Phase 1D` — current runtime assets đã được register vào local BOB registry; checkpoint đã đạt `47/47` assets hiện hành và local `DW_Developer.TableDictionary` đã lên `67` rows sau Phase 1E backfill.
...
- [x] `Phase 1F` — BOB `_Wrk`/wrapper coverage đã canonicalize cho active runtime path; business SQL/view/table/semantic contract giữ nguyên, không rewrite business logic.
...
- [x] `Phase 1H` — final smoke/audit pass: orphan/deprecated objects remaining `0`, BOB incremental staging smoke `Process Complete`, canonical refs clean, freshness/count/grain duplicate checks pass.
...
- [Verified] `Staging_Wrk_Wrk` naming smell đã được canonicalize live không full reload: final target là `Staging.DemandForecastSnapshotDaily`, source wrapper là `Staging_Wrk.v_DemandForecastSnapshotDaily`, và BOB `DW_Developer.usp_IncrementalTableLoad` dùng `DateRange`/`dfcSnapshot`.
...
- Xác nhận local `ETL_Framework` chỉ là partial BOB shell hiện tại.
...
#### Phase 1B — Diff local `ETL_Framework` với full BOB reference

| ID | Bước chi tiết | Thời lượng dự kiến | Output bắt buộc |
|---|---:|---:|---|
| 1B.01 | Export object inventory `EnterpriseData-Dev.ETL_Framework` | 30 phút | reference object list |
| 1B.02 | Export BOB reference definitions cho tables/views/procs/functions cần so sánh | 90-180 phút | BOB DDL reference pack |
...
| 1B.05 | Compare `TableDictionary` column-level schema với expected 65-col BOB contract | 45 phút | column gap list |
| 1B.06 | Compare procs local vs BOB 35-proc families | 60 phút | missing/proc-family matrix |
...
| 1B.09 | Identify duplicate/experimental BOB variants that should not be blindly copied | 30 phút | standardization note |
| 1B.10 | Produce “local-to-full-BOB gap register” with severity: blocker, required, optional, deferred | 60 phút | approved sync backlog |
| 1B.11 | Review gap register with Bob/Rakesh/Aric before any local framework mutation | 60 phút | approval notes |

Exit gate Phase 1B:

- Biết chính xác local `ETL_Framework` thiếu gì để đạt full BOB contract.
...
#### Phase 1C — Nâng local `ETL_Framework` lên full BOB contract
...
- Nhưng exit gate cuối **toàn bộ Phase 1** vẫn giữ nguyên:
  - local `Enterprise SupplyChain-Dev.ETL_Framework` phải cover **100% approved BOB runtime/load contract** cho current SupplyChain scope
  - Bronze / Silver / Gold active paths phải chạy qua BOB path
  - mọi active current load patterns phải có BOB-compatible execution path
  - 04_semantic/report contract vẫn không đổi
- “100% BOB” trong Phase 1 được hiểu là:
  - 100% contract cần cho current Medallion runtime của SupplyChain
  - không bắt buộc copy toàn bộ duplicate/experimental/non-SupplyChain hub variants nếu đã chọn 1 standardized BOB path tương đương và documented rõ object nào được defer như tech debt/out-of-scope
...
| ID | Bước chi tiết | Thời lượng dự kiến | Output bắt buộc |
|---|---:|---:|---|
| 1C.00 | Curate exact Option A sync backlog: map từng hub-only object vào `required-before-phase1-exit` vs `standardized-out/deferred`, và chứng minh current active load patterns vẫn có đường đạt 100% approved BOB coverage khi Phase 1 kết thúc | 60-120 phút | curated Option A sync backlog |
...
| 1C.06 | Approval gate: Aric/Bob/Rakesh xác nhận exact object list sẽ apply | 30-60 phút | explicit approval |
...
| 1C.09 | Validate required BOB proc families callable trong local `ETL_Framework` | 60 phút | proc smoke result |
...
- Local `ETL_Framework` có đủ BOB core objects/procs để chạy SupplyChain Bronze/Silver/Gold.
...
#### Phase 1D — Build metadata mapping từ VN current state sang BOB `TableDictionary`
...
| 1D.02 | Extract current `Meta.TableDictionary` rows đã Bob-align trước đó | 30 phút | VN dictionary extract |
| 1D.03 | Build mapping `AssetRegistry -> BOB TableDictionary` field-by-field | 90-150 phút | mapping workbook/SQL |
...
| 1D.10 | Map owner/source/refresh metadata required by BOB `TableDictionary` | 45 phút | governance mapping |
...
| 1D.12 | Create execution manifest shape: `run_sequence`, `layer`, `project`, `wave_number`, `schema_name`, `object_name`, `depends_on`, `bob_exec_command`, `is_manual_run_enabled` | 60 phút | manifest schema/design |
...
| 1D.15 | Flag rows requiring Bob/Rakesh decision because BOB required fields or dependency order are missing | 30-60 phút | open-decision list |
| 1D.16 | Generate INSERT/MERGE script for new v10 rows into local BOB `TableDictionary` | 90 phút | metadata registration script |
...
- Có mapping đầy đủ từ current v10 objects sang local BOB registry.
- Có registration script không phá metadata cũ.
- Mọi missing BOB-required field được mark rõ.
...
| 1E.07 | Validate local BOB registry can answer: source, target, update method, last modified, row count placeholder | 45 phút | registry query result |
| 1E.08 | Run `usp_UpdateTableDictionary_ModifiedDate` or BOB equivalent on a safe migrated object | 30-45 phút | modified-date smoke |
| 1E.09 | Run `usp_UpdateTableDictionaryModified` or BOB equivalent if required by lifecycle | 30 phút | update-log aggregation smoke |
...
| 1E.12 | Validate manifest uses registered BOB TableDictionary rows, not legacy `SCP_Core` rows | 30 phút | no-legacy-target check |
...
- Local BOB `TableDictionary` trỏ đúng 3 current layers.
- Metadata cũ `SCP_Core` không bị xóa.
- BOB lifecycle update/audit chạy được trên at least one safe migrated object.
- Execution manifest v1 tồn tại và được validate không dùng alphabetical/order-by-name làm thứ tự chạy.

#### Phase 1F — Bọc business SQL bằng BOB `_Wrk`/wrapper pattern, không rewrite
...
| 1F.03 | Create BOB-compatible `_Wrk` view definition bằng cách copy SQL text nguyên văn hoặc reference existing view | 60-120 phút | `_Wrk` view script |
...
| 1F.06 | Create/prepare BOB wrapper call cho pilot object | 45 phút | wrapper call script |
...
| 1F.12 | Capture exceptions where BOB wrapper cannot support current pattern without extension | 45 phút | exception register |

Exit gate Phase 1F:

- Mỗi object migrated có SQL baseline, BOB wrapper path, và parity result.
...
| ID | Bước chi tiết | Thời lượng dự kiến | Output bắt buộc |
|---|---:|---:|---|
| 1G.01 | Confirm no-new-pipeline constraint with Bob/Aric for Phase 1 manual refresh | 15-30 phút | execution constraint note |
| 1G.02 | Define SQL wrapper strategy: existing BOB proc calls only, new SQL wrapper proc if approved, or repo-generated `.sql` run script if no new live proc allowed | 45 phút | wrapper strategy decision |
...
| 1G.10 | Add BOB `AuditLog` start/end/error logging around run-level and object-level execution if supported by local procs | 60-120 phút | audit-wrapped script/proc |
| 1G.11 | Add per-object call to BOB update-log/modified-date proc after successful object refresh | 60 phút | modified tracking integrated |
...
| 1G.19 | Capture duration per object from BOB `AuditLog` and compare with current VN `RunLog` where available | 60-120 phút | runtime comparison |
...
- Bronze/Silver/Gold can run from BOB wrapper entrypoints or generated manual SQL scripts.
...
- `Meta.usp_GenericLoad` is not called in BOB target path.
- BOB logs identify object, command, start/end/error.
...
| ID | Bước chi tiết | Thời lượng dự kiến | Output bắt buộc |
|---|---:|---:|---|
| 1I.01 | Confirm BOB path có đủ runbook: how to run Bronze/Silver/Gold, how to inspect logs, how to rollback | 60 phút | runbook section |
...
| 1I.05 | Review with Aric/Bob/Rakesh | 60-120 phút | signoff notes |
...
| 1I.08 | Package SLA/email alert readiness mapping for SupplyChain BOB rows: `Modified`, `RefreshRate`, `JobServer`, `JobName`, `SchemaName`, `TableName` | 45-60 phút | alert-readiness note |
| 1I.09 | Wait at least 5 calendar days or until Aric starts Phase 2 explicitly | 5 ngày calendar | stability/observation window |

Exit gate Phase 1I:

- Phase 1 BOB path is documented, validated, and reviewable.
...
- [Verified] Local `ETL_Framework` has enough approved BOB core contract for the active SupplyChain runtime path.
...
- [Verified] Local BOB `TableDictionary` registers active approved Silver/Gold curated objects; Bronze/source contracts are represented in mart DQ/catalog docs and source notes.
...
- [Verified] Active load patterns have BOB-compatible execution coverage: overwrite via `usp_RefreshCuratedTableFromView`, incremental/date-key patterns via `usp_IncrementalTableLoad`.
...
- [Verified] BOB wrappers run without calling `Meta.usp_GenericLoad`; `Meta.usp_GenericLoad` remains fallback only.
...
- [Verified] BOB `AuditLog` shows migrated object activity: latest full run has `56 Process Start` + `56 Process Complete`, `0` errors.
...
### Giai đoạn 2 — Port DQ / Data Lineage / Wave sang BOB extensions

Mục tiêu: sau khi BOB baseline chạy ổn, mới nâng cấp các capability VN thật sự đáng giữ vào BOB ecosystem. Không port toàn bộ control plane.
...
- Thiết kế BOB DQ extension:
  - `DQRule`
  - `DQGateRun`
  - fail/warn mode
  - post-load hook vào BOB wrapper.
- Thiết kế BOB Data Lineage extension:
  - source-to-target lineage
  - table/view/proc dependencies
  - 04_semantic/report downstream mapping.
- Thiết kế BOB Wave extension:
  - dependency graph
  - wave number
  - execution group
...
- BOB path vẫn chạy Bronze/Silver/Gold thành công.
- DQ status trả lời được từ BOB-facing surface.
...
Mục tiêu: chuyển operational ownership sang BOB path sau khi BOB baseline và extensions đều pass parity.
...
| Mất năng lực Control Plane | BOB chưa thấy equivalents cho waves/DQ/lineage | operational regression | port DQ/Lineage/Wave vào BOB extensions sau Phase 1 |
...
| Không được tạo pipeline mới | Bob không approve thêm Fabric item | không có manual orchestration surface mới | dùng SQL-only wrapper/manual run script trong local `ETL_Framework`; future pipeline chỉ gọi wrapper nếu được approve |
...
- Giữ VN current path chạy được đến khi BOB path chứng minh parity.
...
| Scheduling | disable BOB trigger mới, chỉ re-enable VN trigger đã biết sau approval |
...
- Manifest có đủ các cột tối thiểu: `run_sequence`, `layer`, `project`, `wave_number`, `schema_name`, `object_name`, `depends_on`, `source_objects`, `bob_exec_command`.
...
- [Need-verify] BOB `TableDictionary` có complete approved rows cho migrated objects.
- [Need-verify] BOB `AuditLog`/`TableDictionary_UpdateLog` reconcile với Fabric job history.
...
- Thêm enterprise-facing ops view trên BOB `TableDictionary + AuditLog + UpdateLog`.
- Document trạng thái rỗng hiện tại của `SupplyChain_Warehouse` và target owner.

### Cải tiến trung hạn

- Populate BOB path side-by-side bằng exact SQL logic cho Bronze/Silver/Gold.
- Port source contracts và DQ gates vào BOB extension ở `WarnOnly`, sau đó `CriticalStops`.
- Thêm BOB-native AuditLog/UpdateLog sync.
...
### Khuyến nghị 1 — Local BOB-first, không hybrid runtime

[Likely] Dùng local `Enterprise SupplyChain-Dev.ETL_Framework` làm nền, nâng lên full BOB contract, apply BOB ETL Framework 100% cho Bronze/Silver/Gold trước, rồi mới port DQ/Data Lineage/Wave như extensions.

Lý do:

- BOB framework là enterprise-recognized operating pattern.
...
- Giữ `Meta.usp_GenericLoad` làm runner chính sẽ khiến migration vẫn là VN framework, chưa phải BOB framework.
- VN có runtime features đáng giữ, nhưng nên port chọn lọc sau khi BOB baseline chạy ổn: DQ, Data Lineage, Wave.
...
1. [Need-verify] Bob/Rakesh muốn `EnterpriseData-Dev.SupplyChain_Warehouse` host chỉ shared Silver, hay cả một số Gold/serving objects?
2. [Need-verify] Wave Planner nên được port sang BOB extension tables hay encode trong BOB wrapper procs?
3. [Need-verify] Schedule owner chính thức là ai: BOB/enterprise scheduler, Fabric schedules, hay hybrid?
...
```text
Local ETL_Framework được nâng lên full BOB
+ BOB ETL Framework 100% cho Bronze/Silver/Gold execution
