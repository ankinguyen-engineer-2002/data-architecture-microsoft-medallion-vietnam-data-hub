...
Usage:
  AZURE_CONFIG_DIR=/private/tmp/azcfg python3 01_docs/runbook/scripts/update_fabric_semantic_model_definition_tmdl.py \\
    --workspace-id <wsGuid> \\
...
def main() -> int:
    ...
    if status == 200:
        ...
        if not location and operation_id:
            location = f"https://api.fabric.microsoft.com/v1/03_operations/{operation_id}"
        if not location:
