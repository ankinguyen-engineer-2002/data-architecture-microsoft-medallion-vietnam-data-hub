...
Usage:
  python3 01_docs/runbook/scripts/export_fabric_semantic_tmdl.py \
...
def get_definition(workspace_id: str, model_id: str, token: str) -> dict[str, Any]:
    ...
    if not operation_url and operation_id:
        operation_url = f"https://api.fabric.microsoft.com/v1/03_operations/{operation_id}"
    ...
    for _ in range(240):
        ...
        if op_status == "succeeded":
            if operation_id:
                result_url = f"https://api.fabric.microsoft.com/v1/03_operations/{operation_id}/result"
