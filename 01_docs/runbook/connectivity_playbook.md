...
Language note: 01_docs/code in English; short Vietnamese hints included where helpful.
...
- `03_operations/apps/lineage_explorer/export_lineage_data.py` (`get_token()` + `connect()`)
...
Examples (see also `99_archive/reverse-engineering/enterprise_supplychain_dev_architect/30_runbook/19_legacy_v8_daily_refresh_recovery.md`):
...
- Pipelines export: `99_archive/reverse-engineering/enterprise_supplychain_dev_architect/05_tools/export_v10_readiness_baseline.py` uses `POST .../items/{id}/getDefinition`
- Semantic model definition (TMDL): `03_operations/apps/lineage_explorer/export_lineage_data.py` uses `POST .../semanticModels/{id}/getDefinition?format=TMDL` and handles 200 vs 202 (LRO)
- Dataflows automation: `99_archive/reverse-engineering/enterprise_supplychain_dev_architect/projects/inventory_health/dataflows/drafts/API_AUTOMATION_RESULTS.md`
...
- Reference payload example: `99_archive/reverse-engineering/enterprise_supplychain_dev_architect/projects/inventory_health/artifacts/backups/semantic_relationship_fix_20260528/update_definition_body.json`
...
Semantic smoke tests (DAX) use `executeQueries` (see `99_archive/reverse-engineering/enterprise_supplychain_dev_architect/projects/*/50_semantic.md`).
...
Evidence in repo:
- `99_archive/reverse-engineering/enterprise_supplychain_dev_architect/projects/inventory_health/00_workspace.md` notes MCP server `fabric-dynamic` reuses `az` token.
- `99_archive/reverse-engineering/enterprise_supplychain_dev_architect/projects/inventory_health/dataflows/drafts/API_AUTOMATION_RESULTS.md` documents:
