...
Related enterprise context (end-to-end infra + orchestration operating model):
...
A **value-stream data platform** on Fabric for Supply Chain: transforms in **Processing (Silver)**, publishes in **Gold**, and serves BI via **Direct Lake 04_semantic/report**.
...
- **Value stream (VN / Enterprise SupplyChain-Dev)**: runtime-first.
  ...
  - An active serving layer (Gold + 04_semantic/report), therefore it owns the “consumer contract”.
...
7) **Serve**: 04_semantic/report consumes Gold; if a drift exists, this is where incidents surface fast.
...
- VN: [Verified] 04_semantic/report active → higher serving ops burden (schema stability, Direct Lake failures, binding drift).
...
- With active 04_semantic/report, the “data contract” is not just the table schema; it includes relationships/measures/visual expectations. Small upstream drift can cause user-facing incidents.
