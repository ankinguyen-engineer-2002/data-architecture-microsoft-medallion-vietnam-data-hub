# Enterprise ETL Framework Guide

Folder này lưu tài liệu Enterprise ETL gửi và cách repo này hiểu/áp dụng vào SupplyChain Phase 1.

Đọc nhanh:

```text
Enterprise ETL docs = tiêu chuẩn và gợi ý kiến trúc.
...
1. Live evidence đã reverse-engineer từ EnterpriseData/Enterprise ETL hub.
2. Verification artifacts của Phase 1 trong repo này.
3. Enterprise ETL guide docs và email notes.
4. Tài liệu cũ v8/v9/v10 trong archive.

Lý do:

- Enterprise ETL docs có thể mô tả pattern hoặc object lý tưởng.
- Live hub cho thấy runtime thật đang dùng loader procedures, wrapper procedures, `TableDictionary`, `AuditLog`, `_Wrk` và `_LOAD`.
- SupplyChain Phase 1 chọn theo live runtime contract, đồng thời giữ các concept quan trọng của Enterprise ETL: audit, metadata, SLA/freshness, `_Wrk`, managed refresh.

## Concept Enterprise ETL Được Áp Dụng Ở Đây
...
| Email alerting | Capability trong Enterprise ETL ecosystem; tích hợp alert channel đầy đủ là backlog vận hành. |
| Restart from failure | Enterprise ETL loader ecosystem hỗ trợ resume/restart; repo lưu manifest để chạy lại theo dependency order. |

## Kết Quả Phase 1

Phase 1 đã đưa runtime hiện tại về hướng Enterprise ETL-aligned:
