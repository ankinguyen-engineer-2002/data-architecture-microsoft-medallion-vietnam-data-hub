...
def assign_waves(nodes: list[dict[str, Any]], edges: list[dict[str, Any]]) -> list[str]:
    ...
    for node in nodes:
        if node.get("layer") in {"Silver", "Gold"}:
            node["wave"] = 0
    ...
    for layer in ("Silver", "Gold"):
        ...
        queue = deque([node_id for node_id, deps in incoming.items() if not deps])
        assigned: dict[str, int] = {node_id: 0 for node_id in queue}
        while queue:
