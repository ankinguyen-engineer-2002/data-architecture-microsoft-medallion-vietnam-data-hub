#!/usr/bin/env python3
"""Remove generated __bob_source wrappers from active SQL view files.
...
def unwrap_bob_source(sql: str) -> tuple[str, bool]:
    header, body = split_header_body(sql)
    match = re.search(r"\b__bob_source\b\s+AS\s*\(", body, flags=re.IGNORECASE)
    if not match:
        return sql, False
    open_idx = body.find("(", match.start())
    close_idx = find_matching_paren(body, open_idx)
    if close_idx is None:
        raise ValueError("cannot find closing parenthesis for __bob_source")
...
def main() -> int:
    ...
    for path in candidates:
            ...
            if "__bob_source" not in text:
                continue
            new_text, did_change = unwrap_bob_source(text)
            if "__bob_source" in new_text:
                raise RuntimeError(f"unwrap left __bob_source in {path}")
            if did_change:
                changed.append(path)
                if args.apply:
