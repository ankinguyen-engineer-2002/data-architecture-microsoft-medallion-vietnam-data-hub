# ETL Framework Alignment — Enterprise ETL's Pattern vs VN Approach

> Detailed comparison + integration plan. **Audience**: VN team + Enterprise ETL's team review.
>
> **Status**: Proposal · **Author**: Aric Nguyen · **Date**: 2026-05-10

## Executive summary

VN team built an ETL control plane in `Enterprise SupplyChain-Dev` workspace **independently** before fully understanding Enterprise ETL's `ETL_Framework` pattern. After scan + ADR-008 alignment work (executed 2026-05-10), the two patterns are **logically equivalent** but differ in implementation philosophy:

- **Enterprise ETL's pattern**: 35 specialized procs, per-table parameter calls, sequential per-domain SP wrappers
- **VN's pattern**: 1 generic SP (`usp_GenericLoad`) covering 8 load patterns, registry-driven dispatch, parallel DAG waves

VN's approach is **arguably more modern and maintainable**, but needs to **integrate with Enterprise ETL's TableDictionary + AuditLog** at the cross-DB sync layer so domain governance + observability work uniformly across the enterprise.

This doc maps every Enterprise ETL pattern to VN's equivalent + identifies integration points.

---

## §1. Side-by-side architecture

| Layer | Enterprise ETL's `ETL_Framework` (`EnterpriseData-Dev`) | VN's `Meta` (`Enterprise SupplyChain-Dev`) |
|-------|------------------------------------------------|---------------------------------------------|
| **Master registry** | `DW_Developer.TableDictionary` (table, 65 cols, manually seeded + per-load updates) | `Meta.TableDictionary` (table, 69 cols = 63 Enterprise ETL compat + 6 VN ext, seeded from AssetRegistry, auto-updated) |
...
### Enterprise ETL's approach: 35 specialized procs
...
### Verdict
VN's approach scales better for VN team's 33-asset registry. Enterprise ETL's approach scales better for 1,165 tables across 5 domain teams (each owns slice). **Both legitimate**.

## §3. Integration points (cross-DB sync)

For Enterprise ETL's team to see VN's load activity in their hub-side tooling (queries against `ETL_Framework.DW_Developer.TableDictionary`), VN must sync:

### Sync direction A: VN → Enterprise ETL
```sql
...
-- Step 2 (NEW after Enterprise ETL unblocks Q1): cross-DB sync to Enterprise ETL hub
...
**Permission needed** (Enterprise ETL Q1):
...
### Sync direction B: Enterprise ETL → VN (optional)
If VN wants to see hub-side Modified state for shared masters (Calendar, ItemMaster), can:
- Schedule periodic SELECT FROM Enterprise ETL's `TableDictionary` → INSERT into VN's `Meta.TableDictionary` shadow
...
Naming for proposed `SupplyChain_Warehouse.<schema>` (TBC by Enterprise ETL):
...
Enterprise ETL expects each domain team to:
...
1. Register asset in `Meta.AssetRegistry` (richer schema than Enterprise ETL's TableDictionary)
...
1. Register asset in `Meta.AssetRegistry` AND auto-replicate to Enterprise ETL's `TableDictionary` (via `usp_UpdateTableDictionary_ModifiedDate`)
2. Write `_Enh` views
3. Call `Meta.usp_GenericLoad` — Enterprise ETL's team sees execution via shared `TableDictionary.Modified` + `AuditLog`

→ VN keeps registry-driven advantage, Enterprise ETL keeps governance visibility.

## §6. What VN must NOT change

- Don't replace `usp_GenericLoad` with Enterprise ETL's per-table EXEC pattern — that's a regression.
- Don't drop `Meta.RunLog` — Enterprise ETL's AuditLog mixes run-level + audit messages, ours is cleaner.
- Don't drop DAG waves / parallel batch=8 — Enterprise ETL's sequential SP is slower.

## §7. What VN should adopt from Enterprise ETL
...
- ⏳ Cross-DB sync to Enterprise ETL's hub (pending Enterprise ETL Q1 permission)
- ⏳ Code review process via ADO `Enterprise Data Services` PRs (pending Q4 access)

## §8. Open questions for Enterprise ETL

| # | Question | Status |
|---|----------|--------|
| Q1 | Grant write permission on `ETL_Framework.DW_Developer.TableDictionary_UpdateLog` + `AuditLog`? | **Pending Enterprise ETL** |
| Q2 | DDL spec for `AuditLog` (Enterprise ETL's actual cols + types)? Want to match exactly | **Pending Enterprise ETL** |
| Q3 | Owner of `TableDictionary` rows for VN tables — VN seeds? Enterprise ETL's team seeds? | **Pending Enterprise ETL** |
| Q4 | Should VN tables be registered with `RefreshRate` from VN's `frequency` col, or with Enterprise ETL-defined rates? | **Pending Enterprise ETL** |

## §9. Cross-refs

- Full ETL framework synthesis: [`../projects/etl_framework/SYNTHESIS.md`](../projects/etl_framework/SYNTHESIS.md)
- ADR-008 (executed): [`../../docs/decisions/ADR-008-enterprise_etl-alignment-naming-and-integration.md`](../../docs/decisions/ADR-008-enterprise_etl-alignment-naming-and-integration.md)
- VN execution artifacts: [`../../Enterprise_SupplyChain_Dev_architect/artifacts/enterprise_etl_alignment_2026-05-10/`](../../Enterprise_SupplyChain_Dev_architect/artifacts/enterprise_etl_alignment_2026-05-10/)
