...
>
> **2026-06-15 drift note:** this page reflects the earlier dedicated inventory semantic phase. The current live v10 workspace audit shows no separate deployed `sc_inventory_health_control_tower` item in the workspace item list; current 04_semantic/report state is documented in [../live_audit_2026-06-15_v10_core_stack.md](../live_audit_2026-06-15_v10_core_stack.md).
>
...
Source-controlled relationship file for Fabric folder deploys: [`04_semantic/relationships.tmdl`](04_semantic/relationships.tmdl). Keep this as a separate `definition/relationships.tmdl` root part when deploying through Fabric `updateDefinition`; otherwise the live model can publish tables without relationships.
...
## DAX measures (see [04_semantic/Measures_DAX.dax](04_semantic/Measures_DAX.dax))
...
Verify:
```bash
grep -c "schemaName: gold" 04_semantic/SemanticModel.tmdl              # → 0
grep -c "schemaName: Shared_DW" 04_semantic/SemanticModel.tmdl          # → 3
grep -c "schemaName: InventoryHealth_DW" 04_semantic/SemanticModel.tmdl # → 3
```
