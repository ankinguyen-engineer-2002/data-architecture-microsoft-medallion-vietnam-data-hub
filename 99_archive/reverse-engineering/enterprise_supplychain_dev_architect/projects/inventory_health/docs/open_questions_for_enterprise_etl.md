...
> **Status (updated 2026-05-22 evening):** 3 Robert sign-offs ⏳ (H1/H5/M3) + 1 NEW Robert Phase 2 ask ⏳ (Logility past-tracking) + 1 Enterprise ETL architecture Q resolved (DimCalendar consolidated) + ~~4 DE US loads~~ → 3 done + 1 workaround OK + 1 deferred Phase 2 + **1 NEW critical finding for DE US** (Daily/Weekly forecast snapshot quality).
>
> **2026-05-22 evening cleanup** (continued from morning DimDate consolidation):
> - **NEW layer** `Staging.DemandForecastSnapshotDaily` (cross-mart cleaned Bronze materialization)
...
> - **Mart A** `ForecastHistory_Enh.v_ForecastDemandMonthly`: source EL.Daily → Staging.DemandForecastSnapshotDaily (clean)
> - **Mart B NEW** `InventoryHistory_Enh.ForecastSnapshotWeeklySat` (replaces Monday-based ForecastSnapshotWeekly):
>   - Saturday filter from Staging.DemandForecastSnapshotDaily
...
**Where in v10**: [etl/gold_views.sql:v_CogsRollingHelper](etl/gold_views.sql) + [04_semantic/Measures_DAX.dax](04_semantic/Measures_DAX.dax) + [04_semantic/SemanticModel.tmdl](04_semantic/SemanticModel.tmdl) — all marked `M3 FIX`.
...
| `MasterData_ItemMaster_AFI.ITEMASA` | (STID, ITNBR) | N/A (out of scope) | Not used in inventory_health views | Not our concern; Enterprise ETL's hub stranded-table issue |
...
## §C. Enterprise ETL architecture — 1 question
...
**Question for Enterprise ETL**: confirm whether `Shared_DW` is the preferred naming/location for reusable Gold dimensions, or whether enterprise convention should use a different shared schema name later.
