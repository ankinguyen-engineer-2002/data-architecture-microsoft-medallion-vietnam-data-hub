# Open Questions for Enterprise ETL — pending reply

Date opened: 2026-05-10
Status: Awaiting Enterprise ETL's response (email not yet sent — draft pending)
...
**Asks Enterprise ETL:**
1. Owner of schema `MasterData_DW` in `MasterData_Warehouse` — is it Enterprise ETL's team, or owned by Retail/Wholesale data products?
...
**Observed:** `EnterpriseData-Dev` (Enterprise ETL's hub `5360a935-…`) does NOT contain a `SupplyChain_Warehouse`. Existing siblings: `Retail_Warehouse`, `Wholesale_Warehouse`, `MasterData_Warehouse`, `Distribution_Warehouse`, `Quality_Warehouse`, `Centralized_Warehouse`.

**Asks Enterprise ETL:**
1. Plan: (a) create fresh `SupplyChain_Warehouse` in `EnterpriseData-Dev` as a sibling, (b) enterprise_etl's team creates and grants VN team contributor rights, (c) some other plan?
...
2. **Azure DevOps `Enterprise Data Services` project access** for VN team — required to align with Enterprise ETL's Git source-of-truth model (workspace `EnterpriseData-Dev` is `ConnectedAndInitialized` to `ashleyfurniture/Enterprise Data Services/Fabric-EnterpriseData`)
...
|------|----------|-----------|
| Casing `_ENH` → `_Enh` | Adopt | Evidence: 7+ schemas in Enterprise ETL's WS use `_Enh` (e.g., `Source_Data.SupplyChain_Enh`, `Retail_Warehouse.MasterData_HR_UKG_Enh`) |
| Casing `_WRK` → `_Wrk` | Adopt | Same — `SalesHistory_AFI_Wrk`, `Retail_Sales_Wrk` |
| `_DW` suffix | Keep ALL CAPS | Enterprise ETL's `MasterData_DW`, `Source_Data.SupplyChain_DW` use ALL CAPS |
| View prefix `vw_` → `v_` | Adopt | All Enterprise ETL's views use `v_*` (e.g., `v_InvoiceDetail`) |
| Local `Meta.AuditLog` clone | Build local | Mirror Enterprise ETL's `ETL_Framework.DW_Developer.AuditLog` schema; sync to Enterprise ETL's via `usp_LogRun` once Q1 unblocks |
| Extended `Meta.vw_TableDictionary` | Add cols | Match Enterprise ETL's 65-col schema (ServerName, DatabaseName, ETLTool, RefreshRate, UpdateMethod, UpdateQuery, etc.) |
| Drop AI feature-store proposal | Drop | Enterprise ETL confirmed AI consumes Semantic Model (per Rich/Louise) |
| Schema name format inside VN WS | Keep current | `SalesHistory_Enh`, `ForecastHistory_Enh`, `OpenOrderHistory_Enh`, `ReferenceMaster_Enh`, `Staging_Wrk` — match Enterprise ETL's domain-pattern style |

→ All above are being executed via `Enterprise_SupplyChain_Dev_architect/artifacts/enterprise_etl_alignment_2026-05-10/` scripts.
