# Future-State Workflow — Build-Direct Pattern (post Enterprise ETL unblock)

> Pre-condition: Enterprise ETL/Rakesh grant scoped Contributor on `EnterpriseData-Dev.SupplyChain_Warehouse.SupplyChain_Silver` + Azure DevOps `Enterprise Data Services` access.

## §1. End-to-End Data Flow (architectural view)

```
┌──────────────────────────────────────────────────────────────────────────────────────────┐
│                          🇺🇸 EnterpriseData-Dev (Enterprise ETL's hub)                              │
│                                                                                            │
...
│  │  ├─ ADF mounted feeds  │   │  └─ DimItemMaster (existing, owned by Enterprise ETL's team)   │   │
│  │  └─ Mirror Databricks  │   │                                                       │   │
...
flowchart TD
    ...
    D1 -->|YES — shared| Hub[Target: Enterprise ETL's hub<br/>EnterpriseData-Dev.SupplyChain_Warehouse.SupplyChain_Silver]
...
sequenceDiagram
    ...
    participant HUB as 🇺🇸 SupplyChain_Warehouse._Silver<br/>(Enterprise ETL's hub)
    ...
    participant ETL as 🇺🇸 ETL_Framework.AuditLog/TableDictionary<br/>(Enterprise ETL's hub)
    ...
    par Asset A — shared (workspace=hub)
        ...
        Log->>ETL: Cross-DB INSERT AuditLog + UPDATE TableDictionary (Enterprise ETL's hub sees update)
    ...
    Log->>HUB: UPDATE Modified col from MAX(LastUpdated) — Enterprise ETL's team sees fresh state
```
...
|----------|------:|-----------|-------|
| Bronze raw (Source_Data 64 schemas) | 636 | 🇺🇸 hub | Enterprise ETL's team |
| Bronze shortcut aggregator (Enterprise_Lakehouse) | 5 schemas | 🇻🇳 SC | Auto (from hub) |
| Silver shared — Master Data | 4 dims (DimDate ext, DimItemMaster, etc.) | 🇺🇸 hub | Enterprise ETL's team primary, VN extension via PR |
...
## §5. Permission matrix (post Enterprise ETL unblock)

| Role | 🇺🇸 EnterpriseData-Dev | 🇻🇳 Enterprise_SupplyChain Dev |
|------|-------------------------|----------------------------------|
| Enterprise ETL / Rakesh team | Admin | Viewer (audit) |
...
Scoping prevents VN team from accidentally touching Enterprise ETL's `MasterData_DW`, `Wholesale_Warehouse`, `ETL_Framework` — write only to designated SC slot.
...
| **1 — Enterprise ETL unblock** | Pending Q1 + Q3 + Q4 | Enterprise ETL grants scoped Contributor + creates `SupplyChain_Warehouse` in hub |
...
| **5 — Tier E retire** | After Enterprise ETL data ready | Drop Staging_Wrk EDW supplements per ADR-002 |
...
| Enterprise ETL's team visibility | ✅ Real-time (data in hub) | ❌ Only post-promote |
...
- ADR-008 — Enterprise ETL alignment (current state — Implemented 2026-05-10)
- ADR-009 — **TBD** — Build-direct future state (write after Enterprise ETL confirms permission model)
- `_open_questions_for_enterprise_etl.md` Q3 — `SupplyChain_Warehouse` creation + Contributor scoping
- Email draft `email_to_enterprise_etl_ankit_2026-05-10.md` Q3
