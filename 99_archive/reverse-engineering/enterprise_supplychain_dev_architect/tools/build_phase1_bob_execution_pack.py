...
def canonical_bob_exec_command(row: dict[str, str]) -> str:
    ...
    for row in manifest_rows:
        ...
        if asset_id == "Staging_Wrk.DemandForecastSnapshotDaily":
            ...
            drift_notes.append(
                {
                    "asset_id": asset_id,
                    "classification": "bob_naming_canonicalized",
                    "reason": "final target moved from Staging_Wrk to Staging; BOB source wrapper remains Staging_Wrk.v_DemandForecastSnapshotDaily",
                }
            )
        new_row["bob_exec_command"] = canonical_bob_exec_command(new_row)
...
def build_manual_run_artifacts(
    ...
    def add_block(filename: str, title: str, rows: list[dict[str, str]]) -> None:
        ...
        for row in rows:
            exec_command = row["bob_exec_command"]
            object_ref = (
...
def write_phase1g_artifacts(manifest_rows: list[dict[str, str]]) -> None:
    ...
    summary = {
        ...
        "register_only_count": sum(
            1 for row in manifest_rows if row["bob_exec_command"].startswith("REGISTER_ONLY::")
        ),
        "approval_required_count": sum(
            1
            for row in manifest_rows
            if (not boolish(row["is_manual_run_enabled"]))
            and not row["bob_exec_command"].startswith("REGISTER_ONLY::")
        ),
...
def build_metadata_sql(manifest_rows: list[dict[str, str]]) -> str:
    lines = [
        "-- Phase 1E metadata backfill for local BOB TableDictionary",
    ...
    for row in manifest_rows:
        ...
        update_query = row["bob_exec_command"]
    ...
    # Destination rows that need executable BOB incremental/datekey metadata.
    for schema, table in [
        ("InventoryHealth_DW", "InventoryClassificationQtyWeekly"),
        ("InventoryHealth_DW", "InventoryHealthSubStatusWeekly"),
        ("InventoryHealth_DW", "FactInventoryHealthFutureWeekEnding"),
