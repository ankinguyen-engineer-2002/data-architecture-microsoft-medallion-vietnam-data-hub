-- Source: SupplyChain_Warehouse.bronze.vw_ref_item_master
-- Object type: VIEW
-- Exported read-only from sys.sql_modules.


CREATE   VIEW bronze.vw_ref_item_master AS
SELECT
    CAST(RowID AS BIGINT)                                AS sk_item,
    ...
    CAST(SeriesGrouping AS VARCHAR(200))                 AS name_series_grouping,
    ...
    ManufacturingStatusChangeDate                        AS dt_manufacturing_status_change,
    ...
    StatusCodeChangeDate                                 AS dt_status_code_change,
    CASE WHEN TRIM(MainPieceItem) = 'True' THEN 1 ELSE 0 END AS is_main_piece,
    CASE WHEN TRIM(StandAloneFlag) = 'True' THEN 1 ELSE 0 END AS is_standalone,
    ...
    DiscontinuedDate                                     AS dt_discontinued,
    SeriesDateArchived                                   AS dt_series_archived,
    ...
    CAST(DivisionRanking AS VARCHAR(200))                AS code_division_ranking,
    CAST(GroupPriceIncr AS VARCHAR(200))                  AS val_group_price_incr,
    TRIM(GroupPricePointType)                             AS code_group_price_point_type,
    TRIM(MarketIntroducedAt)                             AS name_market_introduced_at,
    MarketBeginDate                                      AS dt_market_begin,
    MarketEndDate                                        AS dt_market_end,
