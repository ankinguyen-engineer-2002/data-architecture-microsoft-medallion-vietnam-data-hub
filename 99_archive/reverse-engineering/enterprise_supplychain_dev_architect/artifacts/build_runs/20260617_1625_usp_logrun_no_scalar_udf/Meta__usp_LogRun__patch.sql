CREATE OR ALTER PROCEDURE Meta.usp_LogRun
    @run_id VARCHAR(128),
...
BEGIN
    ...
    -- Inline Central time conversion (replaces scalar UDF Meta.ufn_utc_to_cst)
    DECLARE @cst_offset INT =
        CASE
            WHEN @now >= CAST(DATEADD(DAY, (8 - DATEPART(WEEKDAY, DATEFROMPARTS(YEAR(@now),3,1))) % 7 + 7, DATEFROMPARTS(YEAR(@now),3,1)) AS DATETIME2(6))
             AND @now < CAST(DATEADD(DAY, (8 - DATEPART(WEEKDAY, DATEFROMPARTS(YEAR(@now),11,1))) % 7, DATEFROMPARTS(YEAR(@now),11,1)) AS DATETIME2(6))
            THEN -5
            ELSE -6
        END;
    DECLARE @now_cst DATETIME2(6) = DATEADD(HOUR, @cst_offset, @now);
    ...
    BEGIN
        BEGIN TRY
            ...
            BEGIN
                ...
                -- Do not call any scalar UDFs here (workaround for Fabric pipeline execution context).
